    ...
    public class QueueTransactionProcessor
    {
        ...
        public void ProcessWebHookEvents(NotificationModel eventNotification)
        {
            ...
            if (String.IsNullOrEmpty(lastChangeTokenValue))
            {
                ...
                lastChangeToken.StringValue = string.Format("1;3;{0};{1};-1", eventNotification.Resource, DateTime.Now.AddMinutes(-10).ToUniversalTime().Ticks.ToString());
        ...
        private int ProcessNotificationListItems(WebHookList webHookList, TraceWriter logger, IQueryable<ListItem> unProcessedItems)
        {
            ...
            foreach (ListItem currentItem in unProcessedItems)
            {
                ...
                //  Need to process the update here.
                if (currentItem[WebHookList.WebhookProcessedDate] == null)
                {
                    //  TODO:  Make the required payment
                    currentItem[WebHookList.PaymentStatus] = "Processed";
                    currentItem[WebHookList.WebhookProcessedDate] = System.DateTime.Now;
                    doUpdate = true;
                }
